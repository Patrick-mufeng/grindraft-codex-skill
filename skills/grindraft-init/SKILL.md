...
name: grindraft-init
description: |
  首次 onboarding 与脚手架创建器。检测用户状态 → 创建项目骨架 → 配置参数。
  支持双模式：有历史数据的用户直入 calibration，从零开始的用户走 cold-start。
  触发词："初始化"/"init"/"磨稿初始化"/"首次使用"。
allowed-tools:
  - Bash
  - Read
  - Write
...
| 文件 | 来源模板 | 用途 |
|------|---------|------|
| `rubric_notes.md` | `templates/rubric_notes.template.md` | 评分规则真实来源 |
| `style_guide.md` | `templates/style_guide.template.md` | 个人风格指南 |
| `WORKFLOW.md` | `templates/workflow.template.md` | 工作流文档 |
| `candidates.md` | `templates/candidates.template.md` | 选题池 |
| `benchmark.md` | `templates/benchmark.template.md` | 历史 Benchmark（如无数据则为空壳） |
| `audience.md` | `templates/audience.template.md` | 受众画像（如无数据则为空壳） |
| `.env` | 新建 | API 配置模板 |
| `STATUS.md` | — | grindraft-status 首次运行时创建，init 跳过 |
...
```js
const { scaffold } = await import("./scripts/scaffold.mjs");
...
📂 已创建项目骨架：
  articles/       — 每篇文章一个文件夹（草稿/终稿/封面/配图/排版/预测/复盘）
  cover/          — 封面设计文件
  illustrations/  — 正文配图
...
| 2 | 目录 | 检查各目录存在 | `articles/` `cover/` `illustrations/` 全部存在 |
...
scaffold(projectRoot, {
  ...
  targetFrequency,   // "weekly-2" | "weekly-1" | "weekly-0.5"
  autonomyDefault,   // "low" | "medium" | "high"
  styleMigration,    // boolean
  rubricVersion,     // "v0" | "v1"
...
| 目录 | 用途 |
|------|------|
| `articles/` | 每篇文章一个子目录（草稿/终稿/封面/配图/排版/预测/复盘） |
| `cover/` | 封面设计输出 |
| `illustrations/` | 正文配图输出 |
| `.grindraft-cache/` | 临时缓存 |

### 项目配置文件

| 文件 | 说明 |
|------|------|
| `.grindraft-state.json` | 项目状态文件（真实来源） |
| `rubric_notes.md` | 评分规则（v0 等权起步） |
| `style_guide.md` | 个人风格指南（待用户填充） |
| `WORKFLOW.md` | 磨稿工作流文档 |
| `candidates.md` | 选题池 |
| `benchmark.md` | 历史 Benchmark |
| `audience.md` | 受众画像 |
| `.env` | API 密钥配置 |
...
用户说"磨稿初始化"
  │
  ├─ Phase 0: .grindraft-state.json 存在？
  │   ├─ 是 → 询问"重新配置还是继续？"
  │   │   ├─ 继续 → 退出，提示"用 '状态' 查看"
  │   │   └─ 重新配置 → 读旧 state 作参考 → Phase 1
  │   └─ 否 → Phase 1
  │
  ├─ Phase 1: 收集参数
  │   ├─ Q1: 粉丝数
